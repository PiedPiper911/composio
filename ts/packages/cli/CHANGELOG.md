...
- Add `composio onboard`, which gets a fresh install to a working tool call. It sequences login, connecting an account, and running a first tool by calling the existing `login`, `link`, and `execute` commands, and resumes wherever you left off — resume is derived from durable facts (login state, live connections from the API, and one persisted has-executed flag), never a stored step pointer. `--json` emits a state document on stdout, suppresses every prompt, and never writes data, so an agent can drive the same flow a human sees prompted on stderr. Gate 0 inspects Claude Code / Codex wiring and reports it as a non-blocking advisory; it never installs anything (that stays `composio setup`). `composio execute` on its own satisfies the final step, so nothing has to be replayed.
- Bare `composio` now points at `composio onboard` while onboarding is unfinished, instead of printing the command list. The pointer is local and network-free, goes to stderr so piped stdout is unchanged, and clears permanently on your first successful `composio execute`. `composio --help` is unaffected.
...
- Tell you how to fix a tool call the provider refused. A revoked or expired grant comes back as a successful API call whose body carries the provider's own code (`token_revoked`, `invalid_auth`, `invalid_grant`, `Bad credentials`, …), so there was nothing for the CLI to key on and it printed the raw provider string with no way forward. `composio execute` now recognizes those failures and points at `composio link <toolkit>` to reconnect, and says why `composio connections list` still shows the account as `ACTIVE` — the authorization was withdrawn at the provider, so Composio's record does not change until you reconnect. `composio onboard` reads the same classification instead of guessing: a failed starter demo used to suggest checking `composio connections list`, which reports healthy for exactly this case. The guidance spells out both recovery steps, because `composio link <toolkit>` on its own refuses while the dead connected account still exists — remove it with `composio connections remove <toolkit>` first, then link.
- Print `composio onboard`'s Claude Code / Codex wiring advisories before the first step runs instead of alongside the closing summary, where they landed between a failed step and its explanation and read as part of the diagnosis. A failed starter demo is also narrated once now rather than twice.
- Stop `composio execute` reverting `~/.composio/config.json` changes made by another `composio` process. Every write rewrites the whole shared file, and the merge used a snapshot taken when the process started — so a long-running command could roll back a `developer`, `experimentalFeatures`, or `security` change another process had recorded in the meantime. Writes now re-read the file immediately before merging, and the execute path records its onboarding flag once instead of on every successful execution, so the common command stops writing the file at all after the first time.
- Stop dropping unrecognized top-level keys in `~/.composio/config.json` on every write. Reading the file already preserved keys the running binary's schema does not declare, but writing it back stripped them — so running an older CLI for anything that updates the config silently erased settings a newer one had recorded, and switching back rolled them away. Decode and encode now preserve the same keys, making the config file safe to share between installed CLI versions.
